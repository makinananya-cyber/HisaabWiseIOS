...
@Suite("HomePlaceholderView")
@MainActor
struct HomePlaceholderViewTests {
    ...
    func buildsABodyForEveryState() async throws {
        ...
        _ = HomePlaceholderView(store: store).body
    ...
    func rendersTheServersDisplayString() async throws {
        ...
        // The view's `.loaded` branch is `Text(budget.income.display)`. Asserting the store's value
        // is asserting what reaches the screen: there is no formatting step in between, which is the
        // whole of ADR-0003.
        let rendered = try #require(store.state.value?.income.display)
    ...
    /// Every key `HomePlaceholderView` renders. A key with no copy shows the user the key.
    ...
    func copyComesFromTheStringCatalogue() throws {
        ...
        if let catalogue = Bundle.module.url(forResource: "Localizable", withExtension: "xcstrings") {
            ...
            for key in Self.renderedKeys {
                #expect(Bundle.module.localizedString(forKey: key, value: nil, table: nil) != key)
            }
        }
    }
}
